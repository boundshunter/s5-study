...
def acc_auth(account, password):
    ...
    :return: if passed the authentication , retun the accounts object, otherwise ,return None
    '''
...
def acc_login(user_data, log_obj):  # 执行返回auth ，auth 为用户id 字典信息
    '''
    accounts login func
    ...
    while user_data['is_authenticated'] is not True and retry_count < exit_count:
        ...
        if account == last_account:  # 判断不为空 为空 same_account +1
            same_account += 1
        auth = acc_auth(account, password)  # auth 拿到 认证成功账户id的字典 auth = account_data

        last_account = account  # 初始化 last_account 为当前输入用户
        if auth:  # not None means passed the authentication
            user_data['is_authenticated'] = True
            user_data['account_id'] = account      # 初始化 user_data 字典
            # print("welcome")
            return auth  # 返回用户字典
        retry_count += 1
    else:
...
def acc_check(account):
    ...
    if os.path.isfile(account_file):
        with open(account_file, 'r') as f:
            ...
            status = account_data['status']
            # if status == 8:  # 帐户为管理员
            #     print("\033[31;1mGet accounts [%s] info pemission denied!\033[0m" % accounts)
            #     return False
...
def sign_up():
    ...
    while exist_flag is True:
        ...
        password = input("\033[32;1mpassword:\033[0m").strip()
        exist_flag = acc_check(account)  # json.load(f)
        if exist_flag:  # not none
            print("Account [\033[31;1m%s\033[0m] is exist,try another accounts." % account)
            exist_flag = True
            continue
        else:
            ...
            print("accounts [\033[32;1m%s\033[0m] added sucessed" % account)
            return True
...
def modify():
    ...
    while acc_data is False:
        ...
        if account_data is False:  # 用户不存在
            print("Account [\033[31;1m%s\033[0m] is not exist,try another accounts." % account)
            continue
        else:
